    ...
    <div class="panel panel-info">
        ...
        <div class="panel-body">
            <table class="table table-condensed">
                ...
                <tbody>
                    <?php if($query = $koneksi->query("SELECT * FROM `order` o LEFT JOIN order_detail d ON o.id_order=d.id_order LEFT JOIN barang b ON b.id_barang=d.id_barang WHERE o.status_kirim='belum'")): ?>
                        <?php $no = 1; ?>
                        <?php while($tamu = $query->fetch_array()): ?>
                            <tr>
                                <td><?=$no++?></td>
                                <td><?=$tamu['nama_barang']?></td>
                                <td><?=$tamu['total_bayar']?></td>
                                <td><?=$tamu['tgl_order']?></td>
                            </tr>
                        <?php endwhile ?>
